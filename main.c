...
int main(int argc, char *argv[])
{
    ...
    { /* Start: Data manipulation routines */
        ...
        for(index = 0; index < line; index++)
        {
            ...
            {
                aux_i = clean_line(read_data[index]);
                //printf("%d\t", t_index);
                t_column = 0; 
                for(column = START_OF_LINE; column < (str_column - aux_i); column++)
                {    
                    ch = read_data[index][column];
                    //filter[t_index][t_column] = ch;
                    /* IMPORTANT REMEMBER COMMENT HERE */
                    suppliers[t_index][t_column] = ch;
                    //printf("%c", suppliers[t_index][t_column]);
                    t_column++;
                }
                //printf("\n");
                t_index++;
