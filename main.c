...
int fix_end_of_line(char *read_data[BUFFER_SIZE], int index, int str_column)
{
    ...
    for(t_column = 0, column = (str_column - size_eol); t_column < size_eol, column < str_column; t_column++, column++)
    {
        aux_read_data[t_column] = *read_data[column];
        //printf("%c", aux_read_data[t_index]);
    }
    printf("\n");
    
    if ((strcmp(aux_read_data, FIX_BUG_LINE)) == 0)
    {
        printf("\n\n RETURN SIZE_EOL \n\n");
        return size_eol;
